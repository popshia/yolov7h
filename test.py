...
from utils.plots import output_to_target, plot_images, plot_pred_results, plot_study_txt
...
def test(
    ...
    # REVIEW: change loss size from 3 to 4
    # loss = torch.zeros(3, device=device)
    loss = torch.zeros(4, device=device)
    jdict, stats, ap, ap_class, wandb_images = [], [], [], [], []
    # REVIEW: add random_batch
    ...
    for batch_i, (
        ...
        cv_imgs,
    ) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt:
                ...
                # REVIEW: add rad into txt
                for *xyxy, rad, conf, cls in predn.tolist():
                    xywh = (
                    ...
                    line = (
                        (cls, *xywh, rad, conf) if save_conf else (cls, *xywh)
                    )  # label format
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    pi = (
                        (cls == pred[:, 6]).nonzero(as_tuple=False).view(-1)
                    )  # target indices
