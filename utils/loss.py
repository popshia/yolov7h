...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance, self.SL1rad, self.MSErad = BCEcls, BCEobj, model.gr, h, autobalance, SL1rad, MSErad
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, targets,
                 # REVIEW: extra parameters
                 model=None, num_batchs=-1, epoch=-1, tb_writer=None, cv_imgs=None):  # predictions, targets, model
