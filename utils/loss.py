...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # REVIEW: add extra functions in self assignments
        (
            self.BCEcls,
            self.BCEobj,
            self.gr,
    ...
    def __call__(
        ...
        # REVIEW: add lrad initialization
        lcls, lbox, lobj, lrad = (
        ...
        # REVIEW: add trad and offsets
        tcls, tbox, trad, indices, anchors, offsets = self.build_targets(
            p, targets
        )  # targets

        if tb_writer and num_batchs == 0:
            plot_targets_and_anchors(
                tb_writer, model, epoch, cv_imgs, indices, tbox, trad, anchors, offsets
            )
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    # REVIEW: change lcls index from 5 to 6
        ...
        lobj *= self.hyp["obj"]
        lcls *= self.hyp["cls"]
        # REVIEW: add lrad weight
        lrad *= self.hyp["rad"]
        bs = tobj.shape[0]  # batch size

        # REVIEW: add lrad in loss sum
    ...
    def build_targets(self, p, targets):
        ...
        # REVIEW: add trad and offset_lists
        tcls, tbox, trad, indices, anch, offsets_lists = [], [], [], [], [], []
        # REVIEW: change gain size from 7 to 8
        # gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain
        gain = torch.ones(
            8, device=targets.device
        ...
        # REVIEW: add trad and offsets_lists in return
        return tcls, tbox, trad, indices, anch, offsets_lists
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                # REVIEW: correct cls and obj index
            ...
            # REVIEW: add masking_inboxes for all_rad
            ...
            for i in range(nl):
                ...
                # REVIEW: append mathching radians
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    # )  # targets
                    t_aux = torch.full_like(
                        ps_aux[:, 6:], self.cn, device=device
                    )  # targets
                    t_aux[range(n_aux), selected_tcls_aux] = self.cp
                    # REVIEW: correct cls index
                    # lcls += 0.25 * self.BCEcls(ps_aux[:, 5:], t_aux)  # BCE
                    lcls += 0.25 * self.BCEcls(ps_aux[:, 6:], t_aux)  # BCE

                lrad += self.SL1rad(ps_aux[:, 4], trad_aux)

            # REVIEW: correct obj index
            # obji = self.BCEobj(pi[..., 4], tobj)
            # obji_aux = self.BCEobj(pi_aux[..., 4], tobj_aux)
            obji = self.BCEobj(pi[..., 5], tobj)
            obji_aux = self.BCEobj(pi_aux[..., 5], tobj_aux)
            lobj += (
    ...
    def build_targets2(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                # REVIEW: add activation funtion for prad and append
                prad = fg_pred[:, 4].sigmoid()
                p_rad.append(prad)
