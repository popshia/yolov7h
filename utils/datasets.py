...
class LoadImagesAndLabels(Dataset):  # for training/testing
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        # REVIEW: change label shape warning from 5 to 6
                        # assert l.shape[1] == 5, 'labels require 5 columns each'
                        assert l.shape[1] == 6, "labels require 6 columns each"
                        assert (l >= 0).all(), "negative labels"
                        # TODO: tweak label warning to accept to 2*math.pi
                        assert (l[:, 1:] <= 1).all()
                        # assert (l[:, 1:] <= math.pi*2).all()
                        assert (
    ...
    def __getitem__(self, index):
        ...
        # FIXME: hbb can't do mosiac, disable temporary
        if not self.obb:
            mosaic = False
        if mosaic:
            # Load mosaic
        ...
        if self.augment:
            ...
            if not mosaic:
                # REVIEW: add obb
                if self.obb:
                    img, labels = r_random_perspective(
                        img,
                        labels,
                        degrees=hyp["degrees"],
        ...
        # REVIEW: fix radian value while doing flip lr or ud
        if self.augment:
            # flip up-down
        ...
        # REVIEW: add cv_img while return
...
def load_mosaic(self, index):
    ...
    # REVIEW: add upbound in label index
    ...
    # REVIEW: add obb
    if self.obb:
        img4, labels4 = r_random_perspective(
            img4,
            labels4,
            segments4,
...
def load_mosaic9(self, index):
    ...
    # REVIEW: add obb
    if self.obb:
        img9, labels9 = r_random_perspective(
            img9,
            labels9,
            segments9,
            degrees=self.hyp["degrees"],
...
def r_random_perspective(
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            clockwise_xy = np.concatenate(
                (xy[:, [0, 1]], xy[:, [6, 7]], xy[:, [2, 3]], xy[:, [4, 5]]), axis=1
            )
            ...
            if targets.shape[0] and targets.shape[1] == 10:
                ...
                targets[:, -1] = np.where(
                    targets[:, -1] < 0,
                    1 + targets[:, -1],
                    np.where(targets[:, -1] > 1, targets[:, -1] - 1, targets[:, -1]),
                )
...
def r_box_candidates(box1, imgw, imgh):  # box1(4,n), box2(4,n)
    ...
    corners_outside_img_mask = (
        ((rbox_poly[:, 0::2] < 0) | (rbox_poly[:, 0::2] >= imgw))
        & ((rbox_poly[:, 1::2] < 0) | (rbox_poly[:, 1::2] >= imgh))
    )[:].all(1)

    center_in_img_mask = (
        (0 <= xy[:, 0]) & (xy[:, 0] < imgw) & (0 <= xy[:, 1]) & (xy[:, 1] < imgh)
    )
