...
def plot_images(
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            # REVIEW: change conf condition index from 6 to 7
...
def plot_results(start=0, stop=0, bucket="", id=(), labels=(), save_dir=""):
    ...
    # REVIEW: change subplot size from 2*5 to 2*6
    # fig, ax = plt.subplots(2, 5, figsize=(12, 6), tight_layout=True)
    fig, ax = plt.subplots(2, 6, figsize=(12, 6), tight_layout=True)
    ax = ax.ravel()
    # REVIEW: add 'Radian' and 'val Radian' tags in plot
    s = [
        "Box",
        "Objectness",
        "Classification",
    ...
    for fi, f in enumerate(files):
        try:
            # REVIEW: fix indicies in result column
            # results = np.loadtxt(f, usecols=[2, 3, 4, 8, 9, 12, 13, 14, 10, 11], ndmin=2).T
            results = np.loadtxt(
                f, usecols=[2, 3, 4, 5, 9, 10, 13, 14, 15, 16, 11, 12], ndmin=2
            ).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            # for i in range(10):
            # REVIEW: change for range from 10 to 12
            for i in range(12):
                y = results[i, x]
                # REVIEW: change i range
                # if i in [0, 1, 2, 5, 6, 7]:
                if i in [0, 1, 2, 5, 6, 7, 8, 9]:
...
def plot_targets_and_anchors(
    ...
    for img_id, img in enumerate(cv_imgs):
        ...
        for i, anchor_stride in enumerate(anchor_stride_list):
            ...
            for tbox_id, (box, anchor_wh, offset, rad) in enumerate(
                zip(tbox[i], anchors[i], offsets[i], trad[i])
            ):
                if b[tbox_id] == img_id:
                    box_to_cpu = box.cpu()
                    x, y, w, h = (
                    ...
                    cv2.line(
                        img_with_one_anchor, poly[0], poly[1], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_one_anchor, poly[1], poly[2], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_one_anchor, poly[2], poly[3], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_one_anchor, poly[3], poly[0], color, 1, cv2.LINE_AA
                    )

                    cv2.line(
                        img_with_all_anchors, poly[0], poly[1], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_all_anchors, poly[1], poly[2], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_all_anchors, poly[2], poly[3], color, 1, cv2.LINE_AA
                    )
                    cv2.line(
                        img_with_all_anchors, poly[3], poly[0], color, 1, cv2.LINE_AA
                    )
    ...
    tb_writer.add_image(
        "debug/anchor_all", img_with_all_anchors, dataformats="HWC", global_step=epoch
    )
